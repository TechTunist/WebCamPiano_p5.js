    ...
    for (var i=0;i<this.notePos.length;i++){
      for (var j=0;j<this.notePos[i].length;j++){
        ...
        if (this.noteState[i][j]>0) {
          ...
          var c1 = color(255,0,155,alpha);
          var c2 = color(0,205,255,alpha);
            
            // create slider to edit note colors
            var gridColor = color('hsl('+colorSlider.value()+', 100%, 50%)');
            
          var mix = lerpColor(gridColor, c2, map(i, 0, this.notePos.length, 0, 1));
          fill(mix);
          ...
          // minor random aspect to note position
          this.notePos[i][j].y += random(-1, 1);
